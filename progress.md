...
**Current phase:** **α-gate wins → `gated_fusion` is now the headline HEMT-CLIP; source doc fully reframed.** Test (n=2573): full HEMT-CLIP (α-gate) F1 **0.8393** / Acc **0.8313** / AUC **0.9122** — best of all variants (concat 0.8319/0.9042; α-feature ablation 0.8277/0.8919). User decided (2026-06-01): `gated_fusion` = headline HEMT-CLIP, α-feature variant demoted to ablation; single seed is fine (no seeds 7/123). `report/Chapters_4_5_6_Source.md` rewritten throughout: headline-result box, master-correction item 3 reversed, §4.2.1/§4.3.1 novelty (lead with gated architecture), Table 4.3, §5.1.1/5.1.5/5.1.7 (α-gate is the model, classifier 512), Chapter 6 (Table 6.1 5 rows, §6.3.1/§6.3.3 gate-wins, §6.7). Notebooks 03/04 cells updated with the result. **Next:** user integrates the reframed prose into the docx; Ch.7; slides. Demo is now `notebooks/06_demo.ipynb` (Streamlit/`run_demo.py` removed 2026-06-10 — see entry below).
**Last updated:** 2026-06-11

---

### 2026-06-11 — nb05 explainability made truly multimodal (code scaffolding; awaits Colab re-run)
The XAI gap: cross-attention ran on the headline `gated_fusion`, but **SHAP/LIME ran on a separate `text_only` model** (old Blueprint §10.2 stance) — so word-importance explained a different model than the one we defend. User approved (plan `groovy-herding-orbit`): (1) add a modality-contribution method; (2) keep text_only SHAP/LIME as a *comparison baseline* alongside new multimodal attribution.
- **New `explainability/mm_common.py`** — shared `pick_samples` (de-duplicated from both scripts) + `make_mm_text_predict_fn`: a per-sample multimodal text predict-fn for `gated_fusion` with the image **encoded once and cached**, so SHAP/LIME perturb only the text while the real image + α stay fixed (replays `HEMTCLIP.forward` gated branch via `text_encoder → fusion(…, alpha) → classifier`; mirrors nb06's `_mm_text_predict`). Bypasses re-running CLIP ViT per perturbation.
- **`shap_text.py` / `lime_text.py`** — added `--model {text_only,gated_fusion}` and `--pick-from-npz` (decouples *which samples* from *which model*, so both models explain the SAME rows for comparison); pred/conf now derived live from the explained model; manifests gain a `model` field. Defaults preserve old behaviour.
- **New `explainability/modality_contrib.py`** — per-sample occlusion on `gated_fusion`: `no_image` (pixel=0, α=0 → gate collapses to text) and `no_text` (empty title, real image+α); reports Δlogit_image/Δlogit_text toward the predicted class, prob deltas, and verdict-flip flags. Outputs `outputs/xai/modality/{modality_contrib.png,.csv,modality_manifest.json}` + printed aggregate (mean |Δ| per modality, flip rates, image-dominant count). The genuinely cross-modal evidence.
- **`notebooks/05_explainability.ipynb`** (24 cells) — intro + takeaways reframed to a 4-method truly-multimodal framework (table; reversed the "multimodal SHAP intentionally skipped" caveat). SHAP/LIME cells now run BOTH multimodal (`shap_mm`/`lime_mm`, headline) and text_only (baseline) on the same picks (`--pick-from-npz preds_gated_fusion.npz`); display cells point at the `_mm` dirs. Added a **comparison** section (multimodal-vs-text_only SHAP on shared/disagreeing samples) and **Method 4** (modality contribution run + figure). All code cells outputs cleared (need Colab re-run); pure-python cells ast-parse clean; modules `py_compile` clean.
- **Colab run succeeded (2nd attempt) + Ch 6.4 rewritten from the real outputs.** Headline new result (`modality_contrib`, 30 samples): **mean |Δlogit| image 3.17 vs text 0.48 (~6.6×); image-removal flips 20/30 (67%) verdicts vs text 2/30 (7%); image-dominant 28/30** → the model is strongly **image-dominant** (image = aggressive fake-driver; mechanism behind the recall-skew). Grounded samples replacing the old text_only ones: **1060** (text_only→FAKE 0.52 vocab-bias, multimodal→REAL 0.74 — image corrects it), **2306** (confident FP 0.97; SHAP+LIME tiny/diffuse, Δlogit_image +4.5 ≫ text +0.36 — image-driven error), **1627** (SHAP+LIME agree `crashes` #1, disagree sign on `test`). Rewrote `report/Chapters_4_5_6_Source.md`: §6.4 (5 findings), §6.6 (two error modes: text-bias corrected + image over-flagging), §6.7 pt 7, corrections #10, §4.2.3 table (4 methods) + correction 4&5 note, §4.3 Contribution 2, Appendix A paths, "three-level"→"four-method". Filled nb05 takeaways cell with the same grounded findings. **Caveat noted everywhere:** modality occlusion is OOD → directional magnitudes, corroborated by image-only>text-only AUC.
- **Presentation cleanup of nb05:** stripped the HF `resume_download` FutureWarning + `warnings.warn(` noise lines from the run-cell stream outputs (12 lines; kept all `[INFO]` logs, the 30+30 SHAP/LIME sample lines, and the modality summary), per the 02-05 convention; fixed cell-7 wording (all methods now pick from `preds_gated_fusion.npz`). Final flow: setup → M1 attention → M2 SHAP (mm+baseline) → M3 LIME (mm+baseline) → comparison → M4 modality → grounded takeaways. JSON valid.
- **Fix (after first Colab run crashed):** `make_mm_text_predict_fn` precomputed image patches in fp32 then ran the text/fusion path under fp16 autocast → `nn.MultiheadAttention` mixed-dtype error, and since `base = predict_fn([text])` sits outside the try/except it aborted shap_text on sample 0 (empty `shap_mm/` → display cell `FileNotFoundError` on `shap_manifest.json`). Now runs full fp32 (no autocast), matching nb06's `_mm_text_predict`. **Gotcha:** don't mix cached fp32 features with autocast-fp16 activations in a cached predict-fn.
- **Not done (can't run locally — no torch/GPU):** the actual `_mm`/`modality` outputs, and the **re-derivation of Ch 6.4 sample-specific findings** (Kristallnacht #2221 / tree #1007 / mad-max #2115 were text_only; multimodal picks+attributions differ). Takeaways Findings A/C–E are marked pending the Colab run. Run order on Colab: nb05 top-to-bottom (ensures `preds_gated_fusion.npz`, then attention → SHAP×2 → LIME×2 → comparison → modality).

### 2026-06-11 — Populated `outputs/` from notebook cells + reworked the demo for a defendable viva
Presentation only — no model/result changes.
- **Reconstructed `outputs/` artifacts** from the embedded PNG bytes in the committed notebooks (the Colab FS is gone): cross-referenced each display cell's interleaved markdown headers against the generating cells' "saved …" log lines to recover original filenames. Filled the four existing scaffold folders — `confusion_matrices/` (5 `cm_*`), `attention_examples/` (`attention_grid` + 12 panels), `shap_examples/` (`shap_top_tokens` + 10 per-sample) — plus two new ones: `lime_examples/` (8) and `roc_pr_curves/` (`f1_bar`/`roc_overlay`/`per_class_pr` + `summary_test.csv` rebuilt from nb04's printed table). SHAP/LIME are the *displayed* subsets (10/8 of 30); `training_curves/` stays empty (nb03 has no embedded plots). Dropped a wrong first attempt that used `eval/`+`xai/` paths.
- **Diagnosed the "bad demo":** the OOD examples (made-up titles + Wikimedia images: einstein/lion/green-moon) were the problem, not the model — they're out-of-distribution so the model over-flags FAKE. nb06's own sanity check (cell 14) already proves the pipeline is correct (5/6 labelled-REAL test rows → REAL).
- **Reworked `notebooks/06_demo.ipynb`:** demo now runs on **curated, pre-verified Fakeddit test rows** (`CURATED` = 15662 REAL Eiffel/jeep, 15368 REAL polar bears, **16379 FAKE** "moon" that's really a lamp on a flagpole → hero false-text-image-connection example). Indices vetted from the captured `find_correct_samples()` run; images inspected from the local h5 to pick interpretable/presentable ones. Intro now **defines the Fakeddit 2-way FAKE label** and scopes the demo to the test set; OOD kept once as an explicitly-framed *documented limitation* (cells 17–18); added `demo_custom_title()` (swap title on a real image = safe in-distribution live moment); muted **stop-words** in the headline highlight + plain-English summary. All code cells re-validated (ast-parse OK).
- **New `report/Demo_Script.md`** — ~3-min defense walkthrough (what to say per step) + anticipated Q&A (FAKE-label meaning, OOD/distribution-shift, what α is, why explanations are faithful, fusion-vs-text-only).
- **After user re-ran nb06 on Colab (verified: 3 curated rows correct, attention on the lamp/flag for 16379, sanity 5/6, stop-word fix live):** added a **second FAKE** to `CURATED` — `15657` "student loan debts wiped away" (believable graduation photo + fabricated claim) as a *text-driven* fake complementing 16379's *image-driven* mismatch (vetted from the fresh `find_correct_samples` run; skipped 14644-cat / 15053-xi-jinping). Cell-12 outputs cleared → **needs one re-run on Colab to bake the 4th figure.**
- **New `outputs/headline_metrics_slide.png`** — 16:9 opener composed locally from the populated figures (ROC overlay + gated_fusion confusion matrix + ablation table with gated_fusion row highlighted + F1 0.839 / AUC 0.912 / Acc 0.831 headline).

---

### 2026-06-10 — Final-submission cleanup: notebooks polished, Streamlit removed, demo back in a notebook
For the internal final-submission panel. No model/result changes — presentation only.
- **Removed the Streamlit footprint:** `git rm -r app/` (`streamlit_app.py` + `__init__.py`) and `git rm run_demo.py`; dropped `streamlit`/`pyngrok` from `requirements.txt`. README **Demo** section now points at `notebooks/06_demo.ipynb`; README **Overview** corrected to the final architecture (RoBERTa last-4 layers, **CLIP ViT-B/16** last-4 blocks, 8-head cross-attention, **α-gate** `fused = α·attended + (1−α)·text`, test F1 0.839 / AUC 0.912); repo-layout + ablation row D updated.
- **New `notebooks/06_demo.ipynb`** (self-contained, no `app/` dependency): same Colab bootstrap as 02–05, auto-discovers the `gated_fusion` checkpoint, inline inference helpers ported from the deleted Streamlit app (`preprocess_image`/`compute_alpha`/`predict`/attention overlay), and a single `analyze(title, image, truth=None)` that renders a matplotlib result card (prediction + confidence + α + cross-attention heatmap). Runs on curated REAL/FAKE test samples plus a custom-input cell. **Ships with empty outputs — run once on Colab to bake them in before submission.**
- **Stripped all warnings from notebook outputs (02–05):** removed 298 stderr/warning lines (HF `resume_download`, `torch.cuda.amp.GradScaler`, gradient-checkpointing `requires_grad` UserWarnings) by JSON surgery on the saved outputs; real metric/log lines and figures untouched, per-file indent preserved.
- **Rewrote notebook markdown for presentation:** collapsed nb03's v1→v4 / seed / "α-gate won unexpectedly" archaeology (−387 lines) into a clean final-story account where `gated_fusion` is the headline HEMT-CLIP from the start; nb04 take-aways reframed to lead with the α-gate test win (was still concluding the old "concat beats hemt_clip" story); nb05 draft hedges + `run_demo.py` pointer removed; nb01 α narrative reconciled (we test gate-vs-feature head-to-head and the **gate wins**, rather than the old "α is a feature not a gate"). Removed all emojis, commit hashes, and dev-chatter.
